...
<body>
        ...
        <table style="width:100%">
            <tr>
                ...
                <td style="text-align:right">
                    ...
                    <span class="status-badge badge-{{ $purchase->status->color() }}">{{ $purchase->status->label() }}</span>
                    <span class="status-badge badge-{{ $purchase->payment_status->color() }}">{{ $purchase->payment_status->label() }}</span>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table class="info-table">
        <tr>
            ...
            <td><strong>{{ $purchase->supplier?->name }}</strong><br>{{ $purchase->supplier?->company_name ?: '' }}<br>{{ $purchase->supplier?->full_address }}</td>
            <td class="label">Warehouse:</td>
            <td><strong>{{ $purchase->warehouse?->name }}</strong><br>{{ $purchase->warehouse?->full_address }}</td>
        </tr>
        <tr>
    ...
    <table class="totals">
        <tr><td>Subtotal:</td><td class="text-end">{{ number_format($purchase->subtotal, 2) }}</td></tr>
        @if($purchase->discount_amount > 0)
            <tr><td>Discount:</td><td class="text-end">-{{ number_format($purchase->discount_amount, 2) }}</td></tr>
        @endif
        @if($purchase->tax_amount > 0)
            <tr><td>Tax:</td><td class="text-end">{{ number_format($purchase->tax_amount, 2) }}</td></tr>
        @endif
        @if($purchase->shipping_cost > 0)
            <tr><td>Shipping:</td><td class="text-end">{{ number_format($purchase->shipping_cost, 2) }}</td></tr>
        @endif
        <tr class="grand"><td>Grand Total:</td><td class="text-end">{{ number_format($purchase->total_amount, 2) }}</td></tr>
    </table>

    @if($purchase->notes)
        <div style="margin-top:20px;padding:10px;background:#f9f9f9;border-radius:4px;">
            <strong>Notes:</strong><br>{{ $purchase->notes }}
        </div>
    @endif

    @if($purchase->terms)
        <div style="margin-top:10px;padding:10px;background:#f9f9f9;border-radius:4px;">
            <strong>Terms:</strong><br>{{ $purchase->terms }}
        </div>
    @endif
    ...
    <div class="footer">
        <table style="width:100%">
            <tr>
                <td>Created by: {{ $purchase->creator?->name ?? 'N/A' }}</td>
                <td style="text-align:right">{{ now()->format('d M Y h:i A') }}</td>
            </tr>
        </table>
    </div>
</body>
