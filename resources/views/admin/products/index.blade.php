<x-layouts.admin-layout title="Products">
            ...
            <div class="stat-card-premium stat-border-primary">
                <div class="card-body">
                    ...
                    <div class="stat-bottom">
                        <span class="stat-trend stat-trend-neutral">{{ $statistics['simple'] }} Simple</span>
                        <span class="stat-compare">· {{ $statistics['variable'] }} Variable</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-3 col-6">
            <div class="stat-card-premium stat-border-success">
                <div class="card-body">
                    ...
                    <div class="stat-bottom">
                        <span class="stat-trend stat-trend-neutral">{{ $statistics['draft'] }} Draft</span>
                        <span class="stat-compare">· {{ $statistics['hidden'] }} Hidden</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-3 col-6">
            <div class="stat-card-premium stat-border-warning">
                <div class="card-body">
                    ...
                    <div class="stat-bottom">
                        <span class="stat-trend stat-trend-neutral">{{ $statistics['trending'] }} Trending</span>
                        <span class="stat-compare">· {{ $statistics['best_sellers'] }} Best Seller</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-3 col-6">
            <div class="stat-card-premium stat-border-danger">
                <div class="card-body">
                    ...
                    <div class="stat-bottom">
                        <span class="stat-trend stat-trend-down">{{ $statistics['low_stock'] }} Low</span>
                        <span class="stat-compare">· {{ $statistics['out_of_stock'] }} Out</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.products.index') }}" class="row g-3" id="filterForm">
                <div class="col-md-3">
                    <div class="input-group">
                        <span class="input-group-text bg-white"><i class="fas fa-search"></i></span>
                        <input type="text" name="search" class="form-control product-search"
                               placeholder="Search products..." value="{{ request('search') }}"
                               data-url="{{ route('admin.products.search-suggestions') }}" autocomplete="off">
            ...
            <div class="row g-3 mt-2">
                <div class="col-md-12">
                    <div class="d-flex gap-2 flex-wrap">
                        <a href="{{ route('admin.products.index', array_merge(request()->except(['featured', 'trending', 'best_seller', 'in_stock', 'low_stock', 'out_of_stock', 'with_discount']), ['featured' => 1])) }}"
                           class="badge bg-warning bg-opacity-10 text-warning text-decoration-none py-2 px-3 {{ request('featured') ? 'active' : '' }}">
                            <i class="fas fa-star me-1"></i> Featured
                        </a>
                        <a href="{{ route('admin.products.index', array_merge(request()->except(['featured', 'trending', 'best_seller', 'in_stock', 'low_stock', 'out_of_stock', 'with_discount']), ['trending' => 1])) }}"
                           class="badge bg-secondary bg-opacity-10 text-secondary text-decoration-none py-2 px-3 {{ request('trending') ? 'active' : '' }}">
                            <i class="fas fa-fire me-1"></i> Trending
                        </a>
                        <a href="{{ route('admin.products.index', array_merge(request()->except(['featured', 'trending', 'best_seller', 'in_stock', 'low_stock', 'out_of_stock', 'with_discount']), ['best_seller' => 1])) }}"
                           class="badge bg-success bg-opacity-10 text-success text-decoration-none py-2 px-3 {{ request('best_seller') ? 'active' : '' }}">
                            <i class="fas fa-trophy me-1"></i> Best Seller
                        </a>
                        <a href="{{ route('admin.products.index', array_merge(request()->except(['featured', 'trending', 'best_seller', 'in_stock', 'low_stock', 'out_of_stock', 'with_discount']), ['in_stock' => 1])) }}"
                           class="badge bg-info bg-opacity-10 text-info text-decoration-none py-2 px-3 {{ request('in_stock') ? 'active' : '' }}">
                            <i class="fas fa-box-open me-1"></i> In Stock
                        </a>
                        <a href="{{ route('admin.products.index', array_merge(request()->except(['featured', 'trending', 'best_seller', 'in_stock', 'low_stock', 'out_of_stock', 'with_discount']), ['low_stock' => 1])) }}"
                           class="badge bg-danger bg-opacity-10 text-danger text-decoration-none py-2 px-3 {{ request('low_stock') ? 'active' : '' }}">
                            <i class="fas fa-exclamation-triangle me-1"></i> Low Stock
                        </a>
                        <a href="{{ route('admin.products.index', array_merge(request()->except(['featured', 'trending', 'best_seller', 'in_stock', 'low_stock', 'out_of_stock', 'with_discount']), ['with_discount' => 1])) }}"
                           class="badge bg-primary bg-opacity-10 text-primary text-decoration-none py-2 px-3 {{ request('with_discount') ? 'active' : '' }}">
                            <i class="fas fa-tags me-1"></i> On Sale
                        </a>
                        <a href="{{ route('admin.products.index', array_merge(request()->except(['featured', 'trending', 'best_seller', 'in_stock', 'low_stock', 'out_of_stock', 'with_discount']), ['out_of_stock' => 1])) }}"
                           class="badge bg-dark bg-opacity-10 text-dark text-decoration-none py-2 px-3 {{ request('out_of_stock') ? 'active' : '' }}">
        ...
        <div class="card-body">
                                    ...
                                    <tr data-id="{{ $product->id }}">
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <a href="{{ route('admin.products.edit', $product->id) }}" class="flex-shrink-0">
                                                ...
                                                <div class="min-w-0">
                                                    <a href="{{ route('admin.products.edit', $product->id) }}" class="text-decoration-none text-dark">
                                                        <span class="fw-semibold product-name">{{ $product->name }}</span>
                                                    </a>
                                                    <div class="small text-muted">
                                                        @if($product->brand)
                                                            <span class="badge bg-secondary bg-opacity-10 text-secondary me-1">{{ $product->brand->name }}</span>
                                                        @endif
                                        ...
                                        <td class="text-center">
                                            <div>
                                                <span class="fw-semibold">{{ config('ecommerce.currency_symbol') }}{{ number_format($product->current_price, 2) }}</span>
                                                @if($product->discount > 0)
                                                    <small class="text-muted text-decoration-line-through d-block" style="font-size:10px;">
                                                        {{ config('ecommerce.currency_symbol') }}{{ number_format($product->price, 2) }}
                                                    </small>
                                                @endif
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <span class="badge bg-{{ $product->status->color() }} bg-opacity-10 text-{{ $product->status->color() }} product-status-badge">
                                        ...
                                        <td class="text-end">
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item quick-edit-btn" href="#" data-id="{{ $product->id }}">
                                                            <i class="fas fa-bolt me-2 text-muted"></i> Quick Edit
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a class="dropdown-item" href="{{ route('admin.products.duplicate', $product->id) }}">
                ...
                <div class="d-flex justify-content-between align-items-center mt-4 flex-wrap gap-2">
                    <div class="text-muted small">
                        ...
                        <select name="per_page" onchange="window.location.href=this.value" class="form-select form-select-sm d-inline-block" style="width: auto;">
                            <option value="{{ route('admin.products.index', array_merge(request()->all(), ['per_page' => 15])) }}" {{ $products->perPage() == 15 ? 'selected' : '' }}>15</option>
                            <option value="{{ route('admin.products.index', array_merge(request()->all(), ['per_page' => 25])) }}" {{ $products->perPage() == 25 ? 'selected' : '' }}>25</option>
                            <option value="{{ route('admin.products.index', array_merge(request()->all(), ['per_page' => 50])) }}" {{ $products->perPage() == 50 ? 'selected' : '' }}>50</option>
                            <option value="{{ route('admin.products.index', array_merge(request()->all(), ['per_page' => 100])) }}" {{ $products->perPage() == 100 ? 'selected' : '' }}>100</option>
                        </select>
                        &nbsp;|&nbsp;
                        <div class="btn-group btn-group-sm">
                            <a href="{{ route('admin.products.export.csv', request()->only(['status', 'category_id', 'brand_id', 'product_type', 'search'])) }}"
                               class="btn btn-outline-success" title="Export CSV">
                                <i class="fas fa-file-csv me-1"></i> CSV
                            </a>
                            <a href="{{ route('admin.products.export.excel', request()->only(['status', 'category_id', 'brand_id', 'product_type', 'search'])) }}"
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <p class="text-muted">
                        @if(request()->anyFilled(['search', 'status', 'category_id', 'brand_id', 'featured', 'trending', 'best_seller', 'in_stock', 'low_stock', 'with_discount']))
...
<div class="modal fade" id="bulkEditModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="bulkEditForm" method="POST" action="{{ route('admin.products.bulk-edit') }}">
                @csrf
                <div class="modal-body">
    ...
    if (bulkStatus) {
        bulkStatus.addEventListener('change', function() {
            ...
            if (status && confirm('Change status of selected products to "' + status + '"?')) {
                bulkForm.action = '{{ route('admin.products.bulk-update-status') }}';
