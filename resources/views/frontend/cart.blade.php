<x-layouts.frontend-layout title="Shopping Cart">
            ...
            <div class="col-lg-8">
                <div class="d-flex align-items-center gap-3 mb-4">
                    ...
                    @if ($cart && $cart->items->isNotEmpty())
                        <span class="cart-items-count-badge">{{ $cart->items->sum('quantity') }} items</span>
                    @endif
                </div>
                                ...
                                <div class="cart-item-main">
                                    <div class="cart-item-image">
                                        <a href="{{ $item->product ? route('product.show', $item->product->slug) : '#' }}">
                                            <img src="{{ $item->getProductImage() ?? 'https://placehold.co/120x120/f0f0f0/999?text=N' }}"
                                                 alt="{{ $item->getProductTitle() }}"
                                                 loading="lazy">
                                        </a>
                                    </div>
                                    <div class="cart-item-details">
                                        <a href="{{ $item->product ? route('product.show', $item->product->slug) : '#' }}" class="cart-item-name">
                                            {{ $item->product?->name ?? 'Deleted Product' }}
                                        </a>
                                        @if ($item->variant)
                                            <div class="cart-item-variant">{{ $item->variant->getAttributesList() }}</div>
                                        @endif
                                        <div class="cart-item-mobile-price d-md-none">
                                            <span class="cart-item-current-price">${{ number_format($item->unit_price, 2) }}</span>
                                        </div>
                                        <button class="cart-item-remove-mobile d-md-none" data-item-id="{{ $item->id }}" aria-label="Remove item">
                                            <i class="fas fa-trash-alt"></i>
                                        </button>
                                    </div>
                                    <div class="cart-item-price-cell d-none d-md-block">
                                        <span class="cart-item-price">${{ number_format($item->unit_price, 2) }}</span>
                                        @if ($item->discount > 0)
                                            <span class="cart-item-old-price">${{ number_format($item->unit_price + $item->discount, 2) }}</span>
                                        @endif
                                    </div>
                                    <div class="cart-item-qty-cell">
                                        <div class="cart-qty-selector">
                                            <button class="cart-qty-btn cart-qty-dec" data-item-id="{{ $item->id }}" aria-label="Decrease quantity">-</button>
                                            <input type="text" class="cart-qty-input" value="{{ $item->quantity }}" readonly>
                                            <button class="cart-qty-btn cart-qty-inc" data-item-id="{{ $item->id }}" aria-label="Increase quantity">+</button>
                                        </div>
                                    </div>
                                    <div class="cart-item-total-cell d-none d-md-block">
                                        <span class="cart-item-subtotal" id="subtotal-{{ $item->id }}">${{ number_format($item->subtotal, 2) }}</span>
                                    </div>
                        ...
                        <div class="coupon-row">
                            ...
                            @if ($cart->coupon_id)
                                <div class="applied-coupon">
                                    <span class="applied-coupon-code">{{ $cart->coupon->code ?? 'Coupon' }}</span>
                ...
                @else
                    <div class="empty-cart-state">
                        ...
                        <a href="{{ route('shop') }}" class="btn btn-primary rounded-pill px-4 py-2">Start Shopping</a>
                    </div>
                @endif
            </div>
                    ...
                    <div class="order-summary-card">
                        ...
                        <div class="summary-rows">
                            <div class="summary-row">
                                ...
                                <span class="summary-value" id="summarySubtotal">${{ number_format($cart?->subtotal ?? 0, 2) }}</span>
                            </div>
                            @if ($cart && $cart->coupon_discount > 0)
                                <div class="summary-row discount">
                                    <span class="summary-label">Discount</span>
                                    <span class="summary-value text-success" id="summaryDiscount">-${{ number_format($cart->coupon_discount, 2) }}</span>
                                </div>
                            @endif
                            <div class="summary-row">
                            ...
                            <div class="summary-row">
                                ...
                                <span class="summary-value" id="summaryTax">${{ number_format($cart?->tax_amount ?? 0, 2) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="summary-total">
                            ...
                            <span class="total-value" id="summaryTotal">${{ number_format($cart?->total ?? 0, 2) }}</span>
                        </div>

                        @if ($cart && $cart->items->isNotEmpty())
                            <a href="{{ route('checkout') }}" class="btn-checkout-proceed">
